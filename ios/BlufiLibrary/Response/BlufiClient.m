...
{
    // Re-create the manager to force re-check the Bluetooth status
...
- (NotifyStatus)parseNotification:(NSData *)response notification:(BlufiNotifyData *)notification {
    if (!response) {
        ...
        [_callbackQueue addOperationWithBlock:^{
            if ([self->_blufiDelegate respondsToSelector:@selector(blufiDidReceiveFailedStatus:)]) {
                [self->_blufiDelegate blufiDidReceiveFailedStatus:StatusFailed];
            }
        }];
        return NotifyNull;
    }
